...
class AbetterrouteplannerFlowHandler(
    ...
    async def async_step_reconfigure(
        ...
        """Re-run the OAuth + picker flow to change the tracked vehicles."""
        return await self.async_step_user()
    ...
    async def async_oauth_create_entry(self, data: dict[str, Any]) -> ConfigFlowResult:
        ...
        currently-tracked vehicles and persists the new selection via
    ...
    async def async_step_pick_vehicles(
        ...
        if user_input is not None:
            if user_input[CONF_CAR_IDS]:
                if self.source == SOURCE_RECONFIGURE:
                    ...
                    # poll) doesn't fall out of the decline history mid-
                    # reconfigure. If it reappears later, the auto-add
                    # listener must still treat it as known-but-not-selected.
                    # ``.get(..., [])`` because a legacy entry that hasn't
                    # been reloaded since the upgrade may still lack KNOWN
                    # — setup's migration block normally fills it, but the
                    # reconfigure flow can technically be reached before that
                    # runs.
        ...
        # On first render: default to the current selection when reconfiguring,
        # otherwise suggest selecting all vehicles. On error-driven re-render:
        # preserve the user's last selection so the rejected submission isn't
        # silently overwritten with the default set.
        if user_input is None:
            if self.source == SOURCE_RECONFIGURE:
